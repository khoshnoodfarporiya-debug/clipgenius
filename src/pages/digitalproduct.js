...
export function render(container) {
  ...
  if (iframe && placeholder) {
    iframe.addEventListener('load', () => {
      ...
      try {
        ...
        console.warn('[SSO] Failed to broadcast auth context to Product Agent:', e.message);
      }
    });
  ...
  window.addEventListener('message', (event) => {
    if (event.data && event.data.type === 'AGENT_HEARTBEAT' && event.data.agent === 'digitalproduct') {
      console.log('[SSO] Product Agent heartbeat:', event.data.status);
    }
  });
}
