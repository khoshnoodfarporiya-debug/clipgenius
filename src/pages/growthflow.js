...
export function render(container) {
  ...
  if (iframe && placeholder) {
    iframe.addEventListener('load', () => {
      ...
      try {
        ...
        console.warn('[SSO] Failed to broadcast auth context to GrowthFlow:', e.message);
      }
    });
  ...
  window.addEventListener('message', (event) => {
    if (event.data && event.data.type === 'AGENT_HEARTBEAT' && event.data.agent === 'growthflow') {
      console.log('[SSO] GrowthFlow heartbeat:', event.data.status);
    }
  });
}
