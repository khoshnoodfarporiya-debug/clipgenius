const FALLBACK_SVGS = {
  ...
  'sparkles': `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9.937 15.5A2 2 0 0 0 8.5 14.063l-6.135-1.582a.5.5 0 0 1 0-.962L8.5 9.938A2 2 0 0 0 9.937 8.5l1.582-6.135a.5.5 0 0 1 .963 0L14.063 8.5A2 2 0 0 0 15.5 9.938l6.135 1.581a.5.5 0 0 1 0 .962L15.5 14.063A2 2 0 0 0 14.063 15.5L12.5 21.635a.5.5 0 0 1-.963 0z"/><path d="M20 3h.01"/><path d="M4 20h.01"/></svg>`,
  ...
  'wand-2': `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m15 4 5 5M3 21l9-9M20.96 5.04a2.828 2.828 0 1 0-4 4l4-4z"/></svg>`,
  ...
  'arrow-up-down': `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><polyline points="19 12 12 19 5 12"/></svg>`,
  'edit': `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 1 1 3 3L12 15l-4 1 1-4z"/></svg>`,
...
/**
 * Returns a robust inline SVG string, falling back to embedded SVG if Lucide hasn't loaded.
 * @param {string} name - kebab-case or PascalCase icon name
 * @param {object} attrs - optional attributes like width, height, class, etc.
...
export function getIconSvg(name, attrs = {}) {
  ...
  // 1. Try to query Lucide
  try {
    const pascalKey = toPascalCase(normalizedKey);
    if (window.lucide && lucide.icons) {
      const iconDef = lucide.icons[normalizedKey] || lucide.icons[pascalKey] || lucide.icons[name];
      if (iconDef) {
  ...
  // 2. Use Fallback Local SVG
  const svgContent = FALLBACK_SVGS[normalizedKey] || FALLBACK_SVGS['settings'];

  // Parse SVG and apply attributes
  try {
